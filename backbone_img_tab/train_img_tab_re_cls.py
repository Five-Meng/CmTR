...
def load_model_tabular(opt_dict):
    ...
    # net_pretrain_path = "/data/blood_dvm/data/result/temp/blood/b4_imgq/07_table_75_1e-4_0.6165748018475257.pth"
    net_pretrain_path = "/data/blood_dvm/data/result/temp/blood/b6_imgq/07_table_80_1e-4_0.515610691601595.pth"
...
def load_reconstruction_model(opt_dict, hidden_size2, net_pretrain_net): 
    opt_dict['model_config']['hidden_size2'] = hidden_size2
    model = MLP_Embedding_R(opt_dict)
    weights_dict = torch.load(net_pretrain_net, map_location='cpu')
...
def load_model_image(opt_dict):
    ...
    # net_pretrain_path = "/data/blood_dvm/data/result/temp/blood/b4_imgq/07_image_75_1e-4_0.6165748018475257.pth"
    net_pretrain_path = "/data/blood_dvm/data/result/temp/blood/b6_imgq/07_image_80_1e-4_0.515610691601595.pth"
    weights_dict = torch.load(net_pretrain_path, map_location='cpu')
    model.load_state_dict(weights_dict, strict=True)
    for name, param in model.named_parameters():
        param.requires_grad = False
...
def load_model_align(opt_dict):
    ...
    # net_pretrain_path = "/data/blood_dvm/data/result/temp/blood/b4_imgq/07_align_75_1e-4_0.6165748018475257.pth"
    net_pretrain_path = "/data/blood_dvm/data/result/temp/blood/b6_imgq/07_align_80_1e-4_0.515610691601595.pth"
    weights_dict = torch.load(net_pretrain_path, map_location='cpu')
    # import ipdb;ipdb.set_trace();
    model.load_state_dict(weights_dict, strict=True)
    for name, param in model.named_parameters():
...
def train_img_tab_r_epoch(train_loader, test_loader, device, opt_dict):
    ...
    for epochs in range(50, max_epochs, 5):
        ...
        model_tabular = load_model_tabular(opt_dict)
        model_align = load_model_align(opt_dict)
        # n_tab = "/data/blood_dvm/data/result/temp/blood/b4_imgq/07_tabre_75_1e-4_0.6165748018475257.pth"
        n_tab = "/data/blood_dvm/data/result/temp/blood/b6_imgq/07_tabre_80_1e-4_0.515610691601595.pth"
        model_re = load_reconstruction_model(opt_dict, hidden_size2=opt_dict['model_config']['latent_dim'], net_pretrain_net=n_tab)
...
def train_img_tab_r(model_image, model_tabular, model_re, model_re_img, model_align, train_loader, test_loader, device, opt_dict):
    ...
    for epoch in range(epochs):
        ...
        if(least_val_re > val_losses_re.avg):
            ...
            log_best_re = f"Saved Least Loss_Re: {least_val_re}"
            log_train = write_log(log_train, log_best_re, opt_dict)
            if(val_losses_re.avg < 0.63):
                print("have saved")
                save_image_path = f"/data/blood_dvm/data/result/temp/blood/b2t/07_image_{epochs}_{opt_dict['train_config']['lr_max']}_{least_val_re}.pth"
                save_table_path = f"/data/blood_dvm/data/result/temp/blood/b2t/07_table_{epochs}_{opt_dict['train_config']['lr_max']}_{least_val_re}.pth"
                save_imgre_path = f"/data/blood_dvm/data/result/temp/blood/b2t/07_imgre_{epochs}_{opt_dict['train_config']['lr_max']}_{least_val_re}.pth"
                save_tabre_path = f"/data/blood_dvm/data/result/temp/blood/b2t/07_tabre_{epochs}_{opt_dict['train_config']['lr_max']}_{least_val_re}.pth"
                save_align_path = f"/data/blood_dvm/data/result/temp/blood/b2t/07_align_{epochs}_{opt_dict['train_config']['lr_max']}_{least_val_re}.pth"
...
#     pg_align = [p for p in model_align.parameters() if p.requires_grad]
#     pg = pg_img + pg_tab + pg_re + pg_align + pg_atten
#     optimizer = optim.Adam(pg, lr=float(opt_dict['train_config']['lr_max']))
#     schedule = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=opt_dict['train_config']['epochs'])
...
#         #     lambda_re = 10
#         #     lambda_align = 100
#         # elif(epoch * 3 > epochs and epoch * 3 < epochs * 2):
#         #     lambda_re = 10
#         #     lambda_align = 100
#         # elif(epoch * 3 < epochs):
#         #     lambda_re = 5
...
def train_img_tab_r_atten(model_image, model_tabular, model_re, model_atten, model_align, train_loader, test_loader, device, opt_dict):
    ...
    for epoch in range(epochs):
        ...
        log_out = ('Val_Loss_re {val_loss.val:.4f} ({val_loss.avg:.4f})'.format(
            val_loss=val_losses_re
        ))
        log_train = write_log(log_train, log_out, opt_dict)
        ...
        if(least_val_re > val_losses_re.avg):
            ...
            if(val_losses_re.avg < 0.52):
                print("have saved")
                save_image_path = f"/data/blood_dvm/data/result/temp/blood/b6_imgq2/07_image_{epochs}_{opt_dict['train_config']['lr_max']}_{least_val_re}.pth"
                save_table_path = f"/data/blood_dvm/data/result/temp/blood/b6_imgq2/07_table_{epochs}_{opt_dict['train_config']['lr_max']}_{least_val_re}.pth"
                # save_imgre_path = f"/data/blood_dvm/data/result/temp/blood/b6_imgq2/07_imgre_{epochs}_{opt_dict['train_config']['lr_max']}_{least_val_re}.pth"
                save_tabre_path = f"/data/blood_dvm/data/result/temp/blood/b6_imgq2/07_tabre_{epochs}_{opt_dict['train_config']['lr_max']}_{least_val_re}.pth"
                save_align_path = f"/data/blood_dvm/data/result/temp/blood/b6_imgq2/07_align_{epochs}_{opt_dict['train_config']['lr_max']}_{least_val_re}.pth"
                save_atten_path = f"/data/blood_dvm/data/result/temp/blood/b6_imgq2/07_atten_{epochs}_{opt_dict['train_config']['lr_max']}_{least_val_re}.pth"
...
def train_img_tab_r_add(model_image, model_tabular, model_re, model_re_img, model_align, train_loader, test_loader, device, opt_dict):
    ...
    for epoch in range(epochs):
        ...
        if(least_val_re > val_losses_re.avg):
            ...
            log_best_re = f"Saved Least Loss_Re: {least_val_re}"
            log_train = write_log(log_train, log_best_re, opt_dict)
            if(val_losses_re.avg < 0.62):
                print("have saved")
                save_image_path = f"/data/blood_dvm/data/result/temp/blood/b2/07_image_{epochs}_{opt_dict['train_config']['lr_max']}_{least_val_re}.pth"
                save_table_path = f"/data/blood_dvm/data/result/temp/blood/b2/07_table_{epochs}_{opt_dict['train_config']['lr_max']}_{least_val_re}.pth"
                save_imgre_path = f"/data/blood_dvm/data/result/temp/blood/b2/07_imgre_{epochs}_{opt_dict['train_config']['lr_max']}_{least_val_re}.pth"
                save_tabre_path = f"/data/blood_dvm/data/result/temp/blood/b2/07_tabre_{epochs}_{opt_dict['train_config']['lr_max']}_{least_val_re}.pth"
                save_align_path = f"/data/blood_dvm/data/result/temp/blood/b2/07_align_{epochs}_{opt_dict['train_config']['lr_max']}_{least_val_re}.pth"
                torch.save(model_image.state_dict(), save_image_path)
                torch.save(model_tabular.state_dict(), save_table_path)
                torch.save(model_re_img.state_dict(), save_imgre_path)
                torch.save(model_re.state_dict(), save_tabre_path)
...
if __name__ == '__main__':
    ...
    # model_tabular = load_model_tabular(opt_dict)
    # model_align = load_model_align(opt_dict)
    # n_img = "/data/blood_dvm/data/result/temp/blood/b4_imgq/07_imgre_75_1e-4_0.6165748018475257.pth"
    # n_tab = "/data/blood_dvm/data/result/temp/blood/b4_imgq/07_tabre_75_1e-4_0.6165748018475257.pth"
    # model_re = load_reconstruction_model(opt_dict, hidden_size2=opt_dict['model_config']['latent_dim'], net_pretrain_net=n_tab)
    # model_re_img = load_reconstruction_model(opt_dict, hidden_size2=opt_dict['model_config']['latent_dim'], net_pretrain_net=n_img)
